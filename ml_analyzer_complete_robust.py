#!/usr/bin/env python3
"""
🎯 Machine Learning Analyzer - Dados Reais Completos (Versão Robusta)
...
class RobustCryptoMLAnalyzer:
    def __init__(self):
        self.data_dir = "data/20250829T093234Z"
        self.results_dir = "ml_analysis_complete_robust"
        self.models = {}
        ...
        print("🤖 Iniciando Machine Learning Analyzer - Dados Reais Completos (Robusto)")
        print("=" * 70)
    ...
    def load_all_data(self):
        ...
        # Carregar trades (apenas alguns para evitar sobrecarga)
        trade_files = glob.glob(f"{self.data_dir}/trades/*.json")[:100]  # Limitar a 100
        ...
        # Carregar candles (apenas alguns para evitar sobrecarga)
        candle_files = glob.glob(f"{self.data_dir}/candles/*.json")[:200]  # Limitar a 200
            ...
            if book['data'] and len(book['data']) > 0:
                ...
                if bids and asks:
                    try:
                        ...
                        bid_prices = [self.safe_float_conversion(bid[0]) for bid in bids]
                        bid_sizes = [self.safe_float_conversion(bid[1]) for bid in bids]
                        ask_prices = [self.safe_float_conversion(ask[0]) for ask in asks]
                        ask_sizes = [self.safe_float_conversion(ask[1]) for ask in asks]
                        ...
                        if bid_prices and ask_prices:
                            feature = {
                                ...
                                'spread_pct': ((np.mean(ask_prices) - np.mean(bid_prices)) / np.mean(bid_prices)) * 100 if np.mean(bid_prices) > 0 else 0,
        ...
        for ticker in data['tickers']:
            try:
                feature = {
                    ...
                    'high': self.safe_float_conversion(ticker.get('h', 0)),
                    'low': self.safe_float_conversion(ticker.get('l', 0)),
                    'avg': self.safe_float_conversion(ticker.get('a', 0)),
                    'volume': self.safe_float_conversion(ticker.get('v', 0)),
                    'volume_usd': self.safe_float_conversion(ticker.get('vv', 0)),
                    'change_pct': self.safe_float_conversion(ticker.get('c', 0)),
                    'bid': self.safe_float_conversion(ticker.get('b', 0)),
                    'ask': self.safe_float_conversion(ticker.get('k', 0)),
                    'open_interest': self.safe_float_conversion(ticker.get('oi', 0)),
                    'volatility': abs(self.safe_float_conversion(ticker.get('h', 0)) - self.safe_float_conversion(ticker.get('l', 0))) / max(self.safe_float_conversion(ticker.get('a', 1)), 1),
                    'is_vulnerable': 1 if self.safe_float_conversion(ticker.get('vv', 0)) > 1000000 else 0
                }
                features.append(feature)
            except Exception as e:
                continue
        
        # Features dos trades (simplificado)
                ...
                try:
                    ...
                    for t in trade_data:
                        if isinstance(t, dict):
                            price = self.safe_float_conversion(t.get('p', 0))
                            size = self.safe_float_conversion(t.get('s', 0))
                            if price > 0:
                                prices.append(price)
                            if size > 0:
                                sizes.append(size)
            ...
            try:
                ...
                if volume_usd > 1000000:  # $1M+
                    financial_analysis['high_volume_pairs'].append({
                        'instrument': ticker.get('i', 'Unknown'),
                        'volume': volume_usd,
                        'price': self.safe_float_conversion(ticker.get('a', 0))
                    })
                ...
                low = self.safe_float_conversion(ticker.get('l', 0))
                avg = self.safe_float_conversion(ticker.get('a', 0))
                volatility = abs(high - low) / max(avg, 1) if avg > 0 else 0
    ...
    def generate_markdown_report(self, report, features_df):
        ...
        with open(md_file, 'w', encoding='utf-8') as f:
            f.write("# 🤖 Relatório Abrangente - Machine Learning Crypto.com (Robusto)\n\n")
            ...
            f.write("\n\n---\n")
            f.write("*Relatório gerado automaticamente pelo Machine Learning Analyzer Completo (Robusto)*")
...
def main():
    """Função principal"""
    analyzer = RobustCryptoMLAnalyzer()
